    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <a href="{{ url('budaya/detail/'.$budaya->id) }}" class="flex bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition">
            <div class="w-48 h-40 flex-shrink-0 bg-gray-200 flex items-center justify-center">
                <img src="{{ asset('storage/'.$budaya->gambar) }}" alt="{{ $budaya->judul }}" class="object-cover w-full h-full">
            </div>
            <div class="p-6 flex flex-col justify-between">
                <div>
                    <div class="text-sm text-gray-500 mb-1">Kategori: {{ $budaya->kategori }}</div>
                    <div class="text-xl font-bold text-gray-900 mb-2">{{ $budaya->nama }}</div>
                    <div class="text-gray-600 line-clamp-3">{{ Str::limit(strip_tags($budaya->deskripsi), 120, '...') }}</div>
                </div>
